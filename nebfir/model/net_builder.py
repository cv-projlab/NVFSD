...
from ..trainersV2.trainer_params import TrainerParams

from .net_enums import criterions, models, optimizers, schedulers
...
class Net:
    def __init__(self, netopts):
        self.netopts = netopts
        self.reset()
        
    def reset(self):
        self.device = self.netopts.device
        self.model = get_(self.netopts.architecture_key, models, **self.netopts.architecture_args_dict).to( self.device )
        self.optimizer =  get_(self.netopts.optimizer_key, optimizers, self.model.parameters(), **self.netopts.optimizer_args_dict)
        self.scheduler =  get_(self.netopts.scheduler_key, schedulers, self.optimizer, **self.netopts.scheduler_args_dict)
        self.criterion =  get_(self.netopts.criterion_key, criterions, **self.netopts.criterion_args_dict)

        self.keys = {'architecture': self.netopts.architecture_key, 'optimizer': self.netopts.optimizer_key, 'scheduler': self.netopts.scheduler_key, 'criterion': self.netopts.criterion_key}
